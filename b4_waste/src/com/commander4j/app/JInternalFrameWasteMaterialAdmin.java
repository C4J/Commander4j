...
/**
 * The JInternalFrameWasteMaterialAdmin class allows a user to maintain the
 * table APP_WASTE_REPORT_IDS.
 * 
 * <p>
 * <img alt="" src="./doc-files/JInternalFrameWasteMaterialAdmin.jpg" >
 * 
 * @see com.commander4j.db.JDBWasteMaterial
 * @see com.commander4j.app.JInternalFrameWasteMaterialAdmin
 *
 *
 */
public class JInternalFrameWasteMaterialAdmin extends javax.swing.JInternalFrame
{
	...
	private String lMaterialString;
	...
	{
		JDBWasteMaterial u = new JDBWasteMaterial(Common.selectedHostID, Common.sessionID);
		lMaterialString = JOptionPane.showInputDialog(Common.mainForm, lang.get("dlg_Waste_Material_Add"));


		if (lMaterialString != null)
		{
			if (lMaterialString.equals("") == false)
			{
				lMaterialString = lMaterialString.toUpperCase();
				
				if (u.create(lMaterialString) == false)
				{
					JUtility.errorBeep();
				...
				{
					JLaunchMenu.runForm("FRM_WASTE_MATERIAL_EDIT", lMaterialString);
				}
	...
	private void populateList(String defaultitem)
	{
		...
		for (int j = 0; j < tempReportIDList.size(); j++)
		{
			...
			if (((JDBWasteMaterial) tempReportIDList.get(j).getObject()).getWasteMaterialID().equals(defaultitem))
			{
				sel = j;
			}
	...
	private void renameRecord()
	{
		if (jListReportID.isSelectionEmpty() == false)
		{
			...
			String lMaterialStringgNEW = JOptionPane.showInputDialog(Common.mainForm, lang.get("dlg_Waste_Material_ID_Rename"),lMaterialString);
			...
			if (lMaterialStringgNEW != null)
			{
				if (lMaterialStringgNEW.equals("") == false)
				{
					...
					else
					{
						...
						lMaterialString = lMaterialStringgNEW;
					}
	...
	private void delete()
	{
		if (jListReportID.isSelectionEmpty() == false)
		{
			...
			int question = JOptionPane.showConfirmDialog(Common.mainForm, lang.get("dlg_Waste_Material_Delete") + " " + lMaterialString + " ?", lang.get("dlg_Confirm"), JOptionPane.YES_NO_OPTION, 0, Common.icon_confirm_16x16);
			if (question == 0)
			{
				JDBWasteMaterial u = new JDBWasteMaterial(Common.selectedHostID, Common.sessionID);
				u.setWasteMaterialID(lMaterialString);
	...
	private void print()
	{
		JLaunchReport.runReport("RPT_WASTE_MATERIAL", null, "", null, "");
	}

	private void excel()
	{
		JDBWasteMaterial samp = new JDBWasteMaterial(Common.selectedHostID, Common.sessionID);
		JExcel export = new JExcel();
		export.saveAs("Waste_Reporting_IDs.xls", samp.getWasteMaterialsDataResultSet(), Common.mainForm);
		populateList("");
	}

	public JInternalFrameWasteMaterialAdmin()
	{
		super();
		initGUI();
		...
		try
		{
			...
			this.setTitle("Waste Report ID Admin");
			{
				...
				jDesktopPane1.setLayout(null);
				{
					...
					jScrollPane1.setBounds(0, 0, 677, 512);
					{
						...
						jListReportID.addMouseListener(new MouseAdapter()
						{
							public void mouseClicked(MouseEvent evt)
							{
								...
								{
									if (Common.userList.getUser(Common.sessionID).isModuleAllowed("FRM_WASTE_MATERIAL_EDIT") == true)
									{
										editRecord();
									}
								}
...

						{
...

							{
								...
								newItemMenuItem.setEnabled(Common.userList.getUser(Common.sessionID).isModuleAllowed("FRM_WASTE_MATERIAL_ADD"));
...

							{
								...
								newItemMenuItem.setEnabled(Common.userList.getUser(Common.sessionID).isModuleAllowed("FRM_WASTE_MATERIAL_DELETE"));
								popupMenu.add(newItemMenuItem);
							}

							{
								...
								newItemMenuItem.setEnabled(Common.userList.getUser(Common.sessionID).isModuleAllowed("FRM_WASTE_MATERIAL_EDIT"));
								popupMenu.add(newItemMenuItem);
							}
							{
								...
								newItemMenuItem.setEnabled(Common.userList.getUser(Common.sessionID).isModuleAllowed("FRM_WASTE_MATERIAL_RENAME"));
								popupMenu.add(newItemMenuItem);
							}
							{
				...
				}
				{
					...
					jButtonAdd.setEnabled(Common.userList.getUser(Common.sessionID).isModuleAllowed("FRM_WASTE_MATERIAL_ADD"));
				...
				}
				{
					...
					jButtonDelete.setEnabled(Common.userList.getUser(Common.sessionID).isModuleAllowed("FRM_WASTE_MATERIAL_DELETE"));
				...
				}
				{
					...
					jButtonEdit.setEnabled(Common.userList.getUser(Common.sessionID).isModuleAllowed("FRM_WASTE_MATERIAL_EDIT"));
				...
				}
				{
					...
					jButtonRename.setBounds(689, 93, 125, 32);
					jButtonRename.setEnabled(Common.userList.getUser(Common.sessionID).isModuleAllowed("FRM_WASTE_MATERIAL_RENAME"));
				...
				}
				{
					...
					jButtonPrint.setBounds(689, 124, 125, 32);
				...
				}
				{
					...
					jButtonHelp.setBounds(689, 217, 125, 32);
				}
				{
					...
					jButtonRefresh.setBounds(689, 186, 125, 32);
				...
				}
				{
					...
					jButtonClose.setBounds(689, 248, 125, 32);
...

				{
					...
					jButtonExcel.setBounds(689, 155, 125, 32);
				...
				panel.setBounds(689, 291, 125, 68);
