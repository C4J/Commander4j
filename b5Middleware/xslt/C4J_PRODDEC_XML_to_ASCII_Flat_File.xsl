...
<xsl:stylesheet 
	...
	<xsl:template match="message/messageData/productionDeclaration">
		...
		<row id="1">
			...
			<col id="6"><xsl:value-of select="c4j_XSLT_Ext_formatDate:formatDate(productionDate, 'yyyy-MM-dd''T''HH:mm', 'dd-MMM-yyyy')"/></col>
			<col id="7"><xsl:value-of select="batch"/></col>
			<col id="8"><xsl:value-of select="SSCC"/></col>
			<col id="9"><xsl:value-of select="c4j_XSLT_Ext_formatDate:formatDate(expiryDate, 'yyyy-MM-dd''T''HH:mm', 'dd-MMM-yyyy')" /></col>
			
			<col id="10"><xsl:value-of select="warehouse"/></col>
			
			<col id="11"><xsl:value-of select="storageLocation"/></col>
			<col id="12"><xsl:value-of select="c4j_XSLT_Ext_formatDate:formatDate(expiryDate, 'yyyy-MM-dd''T''HH:mm', 'dd-MMM-yyyy')" /></col>
			<col id="13"><xsl:value-of select="c4j_XSLT_Ext_formatDate:formatDate(productionDate, 'yyyy-MM-dd''T''HH:mm', 'dd-MMM-yyyy')"/></col>
			<col id="14"><xsl:value-of select="old_code"/></col>
			<col id="15"><xsl:value-of select="c4j_XSLT_Ext_subString:subString($sscc, 8, 9)"/></col>
		</row>
		</data>			
	</xsl:template>
