 ...
 */
public class OutgoingProductionUnConfirm
{
	private String hostID;
	private String sessionID;
	final Logger logger = org.apache.logging.log4j.LogManager.getLogger(OutgoingProductionUnConfirm.class);
	private JeMailOutGoingMessage ogm;

	private String errorMessage;
	private JFileIO fio = new JFileIO();

	public OutgoingProductionUnConfirm(String host, String session)
	{
		setHostID(host);
...

	{
		...
		inter.getInterfaceProperties("Production Unconfirm", "Output");
		String device = inter.getDevice();

		JDBPalletHistory palhist = new JDBPalletHistory(getHostID(), getSessionID());
		ResultSet rs = palhist.getInterfacingData(transactionRef, "PROD DEC", "UNCONFIRM", Long.valueOf(1), "SSCC", "asc");
		try
		{
			if (rs.next())
			{
				...
				try
				{
					...
					Element messageType = addElement(document, "interfaceType", "Production Unconfirm");
					...
					productionUnconfirm.appendChild(required_resource);

					Element material = addElement(document, "material", palhist.getPallet().getMaterial());
					productionUnconfirm.appendChild(material);
					...
					if (expiryMode.equals("BATCH") == true)
					{
						...
						productionUnconfirm.appendChild(expiryDate);
					} else
					{
					...
					productionUnconfirm.appendChild(productionUOM);

					Element productionConfirmed = addElement(document, "confirmed", palhist.getPallet().getConfirmed());
					productionUnconfirm.appendChild(productionConfirmed);
					...
					productionUnconfirm.appendChild(productionDate);

					Element messageData = (Element) document.createElement("messageData");
					messageData.appendChild(productionUnconfirm);
					...
					if (device.equals("Disk") | device.equals("Email"))
					{
						...
						if (fio.writeToDisk(path, document, transactionRef, "_" + palhist.getPallet().getLocationID().replace(" ", "_") + "_ProductionUnconfirm.xml") == true)
						{
							result = true;
	...
	public void submit(long dbTransactionRef)
	{
		...
		if (inter.isEnabled() == true)
		{
			...
			ir.write(dbTransactionRef, "Production Unconfirm");
		} else
		{
