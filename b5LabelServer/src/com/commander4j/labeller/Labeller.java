...
public class Labeller extends Thread
{
	...
	public void run()
	{
		...
		File cleanOldtData = new File(prop.getInputPath() + prop.getInputFile());

		if (FileUtils.deleteQuietly(cleanOldtData))
		{
			logger.info("Removing old request file :" + prop.getInputPath() + prop.getInputFile());
		}
		cleanOldtData = null;
		...
		while ((shutdown == false))
		{
			...
			if (requestPrint)
			{
				...
				File renamedInputData = new File(prop.getInputPath() + prop.getInputFile() + ".in");
				FileUtils.deleteQuietly(renamedInputData);
				inputData.renameTo(renamedInputData);

				parseInputData(prop.getInputPath() + prop.getInputFile() + ".in");
				...
				if ((connected) && (shutdown == false))
				{
					...
					if (requestPrint == true)
					{
						...
						{
							/* DELETE FILE */
							if (renamedInputData.delete() == false)
							{
								logger.error("Unable to delete file " + prop.getInputFile() + ".in");
							}
							renamedInputData = null;
							requestPrint = false;
						}
	...
	public Timestamp REMOTE_FILE_DATETIME(String filename)
	{
		Timestamp result = JUtility.getSQLDateTime();
		...
		if (DIR_REMOTE(filename))
		{
			...
			while ((directory.peek() != null))
			{
				LabellerFile labelfile = directory.peek();
				String foundFilename = labelfile.getFilename();
				if (filename.toLowerCase().trim().equals(foundFilename))
				{

					result = labelfile.getDatetime();
					break;
				}
				else
				{
	...
	public boolean SEND_FILE_INTELHEX(String VAL)
	{
		...
		if (result == true)
		{
			...
			try
			{
				...
				String block = ihc.readIntelHexBlock();
				while (block != null)
				{
					rx.clearQueue();
					tx.send(block);
					result = checkSuccess();
					block = ihc.readIntelHexBlock();
				}
				rx.clearQueue();
			...
			if (result == true)
			{
				...
				if (result == true)
				{
					try
					{
						...
						ohc.fromIntelHex(outFile1, outFile2);
	...
	public boolean DIR_REMOTE_FILTER(String VAL)
	{
		...
		while (timeout > 0)
		{
			// System.out.println("Dirtimeout="+String.valueOf(timeout));
			if (lastQSize == rx.responseQueue.size())
			{
				timeout--;
				utils.pause(15);
			}
			...
			{
				lastQSize = rx.responseQueue.size();
				timeout = 50;
			}
		}
